...
Stage 1 calls Groq chat completions (`llama-3.3-70b-versatile`) with the rough transcript and a fixed system prompt asking for a 1–2 sentence description of the recording's topic, setting, and likely proper nouns. *Why:* we already have a working Groq key; this adds no new secret, is fast, and a short summary is well within a small open model's ability. *Alternative considered:* Anthropic Claude — higher quality but introduces a new API key, dependency, and cost. The inference provider/model is isolated in one module (`app/context_inference.py`) so swapping to Claude later is a one-file change.
...
- **Bad/biasing prompt degrades transcription** (Whisper can hallucinate prompt words) → Keep the inferred context short (1–2 sentences), descriptive not directive; the user can trim it. Document this in the inference system prompt.
